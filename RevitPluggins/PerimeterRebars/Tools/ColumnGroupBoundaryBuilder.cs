...
//{
//    // Builds orthogonal spokes from a column group's exterior columns to the floor boundary.
//    // A column is considered exterior when the straight line from its center to the floor
//    // foot does NOT cross any non-adjacent edge of the group's own hull polygon.
//    // Interior columns whose spoke would have to "cut through" the polygon are skipped.
//    public class ColumnGroupBoundaryBuilder
//    {
//        private const double AdjacentEpsilon = 0.01;   // ft — endpoint considered touching the column
//        private const double ParallelEpsilon = 1e-9;
//        private const double CrossingEpsilon = 1e-6;

//        private readonly double _maxDistanceFt;

//        public ColumnGroupBoundaryBuilder(double maxDistanceFt)
//        {
//            _maxDistanceFt = maxDistanceFt;
//        }

//        // Returns the filtered spokes only (caller handles the hull polygon separately).
...
//        // not share the column center as an endpoint (non-adjacent edges only).
